...
Most internationalization libraries rely on raw untyped strings to access their translations. Astro-i18n makes this equally possible; however, it can generate project-specific types to ensure type safety and autocompletion for your translations.
...
When using node (SSR or SSG), you can either follow the same workflow as serverless or store your config and translations on the filesystem.
...
-   The `"common"` group, which is accessible on every page.
...
If there are duplicate keys, `t` will return the more specific one (custom group => local page => common group).
...
All the other locales are supported by your app.
...
Your app translations are in the following format :
...
```jsonc
{
	...
	// routes are also a valid group, they will automatically load on the corresponding page :
	"/posts/[id]": {
		"en": {
			"bye": "Bye"
		},
